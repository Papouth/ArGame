...
public class ImageTargetManager : MonoBehaviour, ISerializationCallbackReceiver
{
    /// <summary>
    ...
    Dictionary<Guid, ImageTargetBehaviour> m_Instantiated = new Dictionary<Guid, ImageTargetBehaviour>();
    ...
    void OnTrackedImagesChanged(ARTrackedImagesChangedEventArgs eventArgs)
    {
        ...
        {
            DeactivateImage(trackedImage);
        }
    }

    void InitImage(ARTrackedImage trackedImage)
    {
        if (!m_Instantiated.TryGetValue(trackedImage.referenceImage.guid, out ImageTargetBehaviour imageTargetBehaviour))
        {
            if (m_PrefabsDictionary.TryGetValue(trackedImage.referenceImage.guid, out var go))
            {
                ...
                {
                    gameObject = Instantiate(go, trackedImage.transform);
                }
                imageTargetBehaviour = gameObject.GetComponent<ImageTargetBehaviour>();
                m_Instantiated[trackedImage.referenceImage.guid] = imageTargetBehaviour;
            }
        }
        imageTargetBehaviour.OnTargetCreated(trackedImage);
    }
    ...
    void UpdateImage(ARTrackedImage trackedImage)
    {
        if (m_Instantiated.TryGetValue(trackedImage.referenceImage.guid, out ImageTargetBehaviour imageTargetBehaviour))
        {
            imageTargetBehaviour.OnTargetUpdated(trackedImage);
        }
    }

    void DeactivateImage(ARTrackedImage trackedImage)
    {
        if (m_Instantiated.TryGetValue(trackedImage.referenceImage.guid, out ImageTargetBehaviour imageTargetBehaviour))
        {
            imageTargetBehaviour.OnTargetRemoved(trackedImage);
        }
    }
    ...
    public void SetPrefabForReferenceImage(XRReferenceImage referenceImage, GameObject alternativePrefab)
    {
        ...
        {
            m_Instantiated[referenceImage.guid] = Instantiate(alternativePrefab, instantiatedPrefab.transform.parent).GetComponent<ImageTargetBehaviour>();//TODO check si component is found sinon le créer
    ...
    /// </summary>
    [CustomEditor(typeof(ImageTargetManager))]
    class ImageTargetManagerInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var behaviour = serializedObject.targetObject as ImageTargetManager;
            ...
            if (m_IsExpanded)
            {
                using (new EditorGUI.IndentLevelScope())
                {
                    ...
                    if (library)
                    {
                        ...
                        {
                            var prefab = (GameObject)EditorGUILayout.ObjectField(image.name, behaviour.m_PrefabsDictionary[image.guid], typeof(GameObject), true/*TODO false*/);
